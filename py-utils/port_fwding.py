...
class Receiver(asyncore.dispatcher):
    ...
    def handle_read(self):
        ...
        self.from_remote_buffers += read

    def writable(self):
        return (len(self.to_remote_buffers) > 0)

    def handle_write(self):
        sent = self.send(self.to_remote_buffers)
        self.to_remote_buffers = self.to_remote_buffers[sent:]
...
class Sender(asyncore.dispatcher):
    ...
    def handle_read(self):
        read = self.recv(BUFSIZE)
        self.receiver.to_remote_buffers += read

    def writable(self):
        return len(self.receiver.from_remote_buffers) > 0

    def handle_write(self):
        sent = self.send(self.receiver.from_remote_buffers)
        self.receiver.from_remote_buffers = \
            self.receiver.from_remote_buffers[sent:]
